...
export function NonRestoringAlgorithm() {
  ...
  const calculateNonRestoring = (q: string, m: string) => {
    ...
    for (let i = 0; i < bitLength; i++) {
      ...
      // If A is negative, add M; if A is positive, subtract M
      if (A[0] === "1") {
        // A is negative, add M
        A = addBinary(A, M)
        ...
        newSteps.push({
          iteration: i + 1,
          substep: 2,
          action: "A is negative, A = A + M",
          a: A,
          q: Q,
          explanation: "A is negative, add divisor to A.",
        })
      } else {
        ...
        newSteps.push({
          iteration: i + 1,
          substep: 2,
          action: "A is positive, A = A - M",
          a: A,
          q: Q,
          explanation: "A is positive, subtract divisor from A.",
        })
      }

      // Set Q[0] based on sign of A
      if (A[0] === "1") {
        // A is negative
        Q = Q.slice(0, -1) + "0"
      } else {
      ...
      newSteps.push({
        iteration: i + 1,
        substep: 3,
        action: `Set Q[n-1] = ${Q[Q.length - 1]}`,
        a: A,
        q: Q,
        explanation: `Set Q[n-1] = ${Q[Q.length - 1]} based on sign of A.`,
      })
    }

    // If A is negative after the last iteration, perform correction
    if (A[0] === "1") {
      A = addBinary(A, M)

      newSteps.push({
        iteration: bitLength + 1,
        substep: 1,
        action: "Final correction: A = A + M",
        a: A,
        q: Q,
        explanation: "A is negative after last iteration, add divisor to A for correction.",
      })
    }
  ...
  <>
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Enter two decimal numbers to see the step-by-step division process using the non-restoring division algorithm.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          {!calculated ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                  {[4, 8, 16].map((length) => (
                    <Button
                      key={length}
                      className={`${bitLength===length && "bg-gradient-to-r from-purple-500/80 to-purple-600/80 hover:from-purple-500 hover:to-purple-600"}`}
              ...
              <Button onClick={handleCalculate} className="w-full bg-gradient-to-r from-purple-500/80 to-purple-600/80 hover:from-purple-500 hover:to-purple-600">
                Calculate
              </Button>
            </motion.div>
          ) : (
                                ...
                                {steps.length > 0 &&
                                  steps[steps.length - 1].a.split("").map((bit, idx) => (
                                    <motion.div
                                      ...
                                      className={`w-8 h-8 flex items-center justify-center border ${
                                        idx === 0 ? "border-l-2 border-l-blue-500" : ""
                                      }`}
                                    >
                                      {bit}
                                    </motion.div>
                                ...
                                {steps.length > 0 &&
                                  steps[steps.length - 1].q.split("").map((bit, idx) => (
                                    <motion.div
                                      ...
                                      className={`w-8 h-8 flex items-center justify-center border ${
                                        idx === steps[steps.length - 1].q.length - 1
